    ...
    try {
      ...
      if (values.teacherIds?.length > 0) {
        for (const teacherId of values.teacherIds) {
          conflictChecks.push(
            schedulesAPI.checkTeacherAvailability(teacherId, dayIndex, slotIndex)
              .then(response => ({
                type: 'teacher',
                id: teacherId,
      ...
      if (duration > 1 && spannedSlots.length > 1) {
        ...
        for (let i = 1; i < spannedSlots.length; i++) {
          ...
          if (values.teacherIds?.length > 0) {
            for (const teacherId of values.teacherIds) {
              spannedConflictChecks.push(
                schedulesAPI.checkTeacherAvailability(teacherId, dayIndex, currentSlotId)
                  .then(response => ({
                    type: 'teacher',
                    id: teacherId,
                    slotIndex: i,
          ...
          if (values.roomId) {
            spannedConflictChecks.push(
              routinesAPI.checkRoomAvailability(values.roomId, dayIndex, currentSlotId)
                .then(response => ({
                  type: 'room',
                  id: values.roomId,
                  slotIndex: i,
